...
public class SimpleLog {
     ...
     * All logs that would not be printed and are above the given logLevel are printed to that File.
     * If you want log Logs printed to sout and serr to file(s), use {@link #addFileLogs(java.io.File, java.io.File)}
     * @param logLevel
    ...
    /**
     * Removes all File-logs created via {@link #addFileLog(com.raxixor.simplelog.SimpleLog.Level, java.io.File)} with given Level.
    ...
    /**
     * Removes all File-logs created via {@link #addFileLog(com.raxixor.simplelog.SimpleLog.Level, java.io.File)} with given File.
     * To remove the sout and serr logs, call {@link #addFileLogs(java.io.File, java.io.File)} with null args.
     *
     * @param file
     ...
     * If this level is set to null, the global Log-level ({@link com.raxixor.simplelog.SimpleLog#LEVEL}) will be used
     *
    ...
    public void log(Level level, Object msg) {
        ...
        format = format.replace("%time%", DFORMAT.format(new Date()))
                .replace("%level%", level.getTag())
                .replace("%name%", name)
                .replace("%text%", String.valueOf(msg));
    ...
    /**
     * Will LOG a message with the trace level.
     *
    ...
    /**
     * Will LOG a message with the debug level
     *
     * @param msg the object, which should be logged
     */
    ...
    /**
     * Will LOG a message with the info level
     *
     * @param msg the object, which should be logged
     */
    ...
    /**
     * Will LOG a message with the warning level
     *
     * @param msg the object, which should be logged
     */
    ...
    /**
     * Will LOG a message with the fatal level
     *
     * @param msg the object, which should be logged
     */
    ...
    /**
     * This interface has to be able to register (via {@link com.raxixor.simplelog.SimpleLog#addListener(com.raxixor.simplelog.SimpleLog.LogListener)}) and listen to log-messages.
     */
    ...
    public enum Level {
        ...
        /**
         * Returns the numeric priority of this LOG-level, with 0 being the lowest
         *
